...
export class AuthService {
    ...
    async signUp(user: CreateUserDto): Promise<Omit<User, 'password'>> {
        ...
        const userFound = await this.userRepository.findOne({where: {email: user.email, password: user.password}});
        if (userFound) {
    ...
    async signIn(credentials: LoginUserDto) {
      const user = await this.userRepository.findOne({where: {email: credentials.email},select:['id','email','password']});
      if (!user) {
